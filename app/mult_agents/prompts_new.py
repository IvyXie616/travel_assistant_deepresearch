...
重要（ChatPromptTemplate 陷阱）：
    ChatPromptTemplate 用 str.format() 解析模板，会把所有 {...} 当作变量占位符。
    因此 prompt 里的 JSON 示例花括号必须用双花括号转义 {{...}}，
    str.format 会把 {{ 还原为 {，}} 还原为 }，LLM 看到的仍是正常单花括号 JSON。
...
WEATHER_PROMPT = """你是旅行出游的天气查询助手，你将收到天气工具返回的文本，该文本包含JSON片段。请从中提取关键天气信息并给出旅行建议。
...
RESEARCH_PROMPT = """你是旅行出游的景点研究助手，你将收到RAG检索工具返回的旅游信息（包含景点名、地址、经纬度、开放时间、门票价格等信息），请提取关键景点信息并给出游览建议。
